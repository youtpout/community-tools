...
if $TESTNET; then
    ...
    # sleep 20
    echo "running on testnet, call the daemon with: ~/Hydra/bin/./hydra-cli -rpcuser=youtpout -rpcpassword=youtpout1234 -testnet getinfo"
else
    ./hydrad -daemon -staking=${STAKING}
    # sleep 20
    echo "running on mainnet, call the daemon with: ~/Hydra/bin/./hydra-cli -rpcuser=youtpout -rpcpassword=youtpout1234 getinfo"
fi
